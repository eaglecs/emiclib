...
class GetListReserveQueueUseCase(useCaseExecution: UseCaseExecution, private val appRepository: AppRepository) : UseCaseOutput<List<ReserveResponse>, ErrorResponse>(useCaseExecution) {
    override fun buildUseCaseObservable(): Observable<List<ReserveResponse>> {
        return appRepository.getListReserveQueue()
    }
