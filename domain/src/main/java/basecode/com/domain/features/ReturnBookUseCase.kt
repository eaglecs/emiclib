...
class ReturnBookUseCase(useCaseExecution: UseCaseExecution, private val appRepository: AppRepository) : UseCase<String,BorrowReturnBookResponse, ErrorResponse>(useCaseExecution) {
    override fun buildUseCaseObservable(input: String): Observable<BorrowReturnBookResponse> {
        return appRepository.returnBook(input)
    }
