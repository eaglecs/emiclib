...
class ReserverBookUseCase(useCaseExecution: UseCaseExecution, private val appRepository: AppRepository) : UseCase<Long, Int, ErrorResponse>(useCaseExecution) {
    override fun buildUseCaseObservable(input: Long): Observable<Int> {
        return appRepository.reserverBook(input)
    }
